...
- **Headers**: `sticky-header-mobile` for sticky section headers (uses `--tab-header-height` CSS variable, dynamically measured via ResizeObserver), `sticky-header-below-progress` modifier for headers below a sticky progress bar (uses `--sticky-content-top` CSS variable)
...
- **Print**: `print-container` (hidden on screen, visible in `@media print`), `print-title`, `print-table`, `print-th`, `print-th-day`, `print-row`, `print-td`, `print-td-day`, `print-meal-name`, `print-meal-desc`, `print-meal-empty` — renders the weekly plan as a compact landscape table optimized for one US Letter sheet; `print-grocery-columns`, `print-grocery-category`, `print-grocery-category-title`, `print-grocery-items`, `print-grocery-item`, `print-grocery-checkbox`, `print-grocery-checkbox-checked`, `print-grocery-name`, `print-grocery-name-checked`, `print-grocery-qty` — renders the shopping list as a multi-column checklist in portrait; `print-prep-day`, `print-prep-day-title`, `print-prep-tasks`, `print-prep-task`, `print-prep-checkbox`, `print-prep-checkbox-checked`, `print-prep-task-text`, `print-prep-task-checked`, `print-prep-meals` — renders the prep plan as a day-grouped checklist in portrait
